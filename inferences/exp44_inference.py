...
STOP_WORDS_LIST = load_resources("general", "stop_words_list", "joblib")
...
def predict(data):
    ...
    vectorized_region = np.vectorize(region)

    x = vectorized_preprocess(np.array(data))
    region = vectorized_region(np.array(data))
    ...
    if y_pred[0][0][0] is not None:
        ...
        return (
            f"Строка для поиска: **{x}**, регион: **{region}**",
            f"Результат поиска: "
